...
class MainWindowRobin:
    ...
    def simulate_round_robin(self):
        ...
        while True:
            ...
            print(
                f"Inicia la tarea en el ciclo {self.clock_cycle}..........  {process}"
            )
            ...
            while task_cycles <= self.quantum:
                ...
                if not self.bloqued_processes:  
                    ...
                    if process_blocked["time_blocked"] == 0:
                        self.remove_block_process()
                        self.bloqued_processes.pop(0)
                        self.show_details(f"TERMINO BLOQUEDO DE {process_blocked["name"]}, Se agrega a  la cola de tareas el proceso...")
                        time.sleep(2)
                        self.prepared_queue.put(process_blocked)
                        self.show_process(process_blocked)
                ...
                if task_cycles == self.quantum and process["completed"] != True:
                    ...
                    self.show_details(f"NO TERMINO {process["name"]}, Se agrega a la cola de tareas el proceso...")
                    self.remove_cpu()
                    self.clear_timeToComplete()
                    time.sleep(2)
                    print("")
    ...
    def rewrite_bloqued_list(self,message):
        self.mutex.lock()
        self.wigets.listaBloqueados.clear()
        for (data) in message:
            print(data)
            self.wigets.listaBloqueados.addItem(f"{data["name"]} / Burst: {data["burst_time"]} / Arrival: {data["arrival_time"]}")
        self.mutex.unlock()

    def clear_totalList(self):
        self.mutex.lock()
        self.wigets.listaListos.clear()
        self.mutex.unlock()
